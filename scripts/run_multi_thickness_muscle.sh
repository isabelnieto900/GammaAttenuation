...
for thickness in "${THICKNESS_VALUES[@]}"; do
    MAC_FILE="../mac/thickness_concrete_${thickness}.mac"
    DATA_FILE="../results/data_thickness_concrete_${thickness}.root"
    ...
    if [ ! -f "$MAC_FILE" ]; then
        ...
        cat > "$MAC_FILE" << EOF
# Configuración para concreto - ${thickness}cm
/control/verbose 0
/run/verbose 0
/event/verbose 0
/tracking/verbose 0
...
EOF
    ...
    if [ -f "../results/data_run_concrete.root" ]; then
        mv "../results/data_run_concrete.root" "$DATA_FILE"
...
root -l -b -q "analysis/multi_thickness_concrete_analysis.C"
...
python3 analysis/plot_multi_thickness_concrete.py
...
echo "  - thickness_concrete_analysis_data.csv"
echo "  - fit_concrete_results.txt"
echo "  - thickness_concrete_analysis_elegant.png"
echo "  - thickness_concrete_analysis_elegant.pdf"
echo "  - thickness_concrete_analysis_elegant.svg"
echo ""