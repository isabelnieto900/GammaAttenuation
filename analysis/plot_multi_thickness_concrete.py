#!/usr/bin/env python3
"""
Visualización Análisis de atenuación gamma vs espesor para concreto
...
def main():
    ...
    data_file = "/home/isabel/GammaAttenuation/results/multi_thickness/thickness_concrete_analysis_data.csv"
    results_file = "/home/isabel/GammaAttenuation/results/multi_thickness/fit_concrete_results.txt"
    
    if not os.path.exists(data_file):
        print(f"INFO: Datos de multi-espesor para concreto no encontrados.")
        print(f"Para generar estos datos, ejecutar:")
        print(f"./scripts/run_multi_thickness_concrete.sh")
        print(f"./build/gammaAtt analysis/multi_thickness_concrete_analysis.C")
        return
    ...
    mu_measured = 0.0358  # valor actualizado para concreto
    mu_error = 0.0002
    ...
    fig.suptitle('Análisis Multi-Espesor - Atenuación Gamma en Concreto Esquelético', 
    ...
    nist_mu = 0.0781  # cm⁻¹ para concreto a 662 keV (NIST)
    
    stats_text = f"""
    RESULTADOS DEL ANÁLISIS
    =====================
    
    Material: Concreto Esquelético
    ...
    base_name = "results/multi_thickness/thickness_concrete_analysis_elegant"
